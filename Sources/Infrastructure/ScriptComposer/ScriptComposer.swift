    ...
    static func visionPrompt(transcript: [TranscriptSegment], keyframes: [SampledFrame], onScreenText: [FrameText]) -> String {
        ...
        只输出一个合法 JSON 对象，不要 Markdown，不要解释。字段严格如下：
        {
          ...
          "hookStructure": {
            ...
            "payoff": "爆点/反转/信息增量，可为空；注意很多梗视频把爆点放在最后一句的反转里，不要漏掉结尾",
        ...
        - 每张关键帧只用于描述覆盖其时间戳的分镜；不要把某一帧的画面细节写进其它时间段的分镜；
        - 先结合台词与画面文字判断题材背景（电竞/体育/影视/职场等的战队、选手、赛事、黑话），按题材含义理解剧情；转写与字幕冲突时以字幕用词为准，不要把听错的词当字面意思编故事；
        - 分析钩子/爆点时通读全部台词到最后一句：结尾的反转、点题或吐槽往往才是真正的爆点；
