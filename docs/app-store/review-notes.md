...
1. 打开 App →「设置」→ 顶部「AI 模式」选择「云端」。
2. 在「云端 AI 服务」填入以下测试配置:
...
- 审核步骤使用的是 `cloudStandard`:不会上传完整视频。完整视频/云端 ASR 的 `cloudDeep` 只有在非私密能力探测通过并由用户单独确认上传和费用上限后才启用;否则界面与结果会明确记录降级模式。
...
**To test the main flow, use Cloud mode:** Settings → AI Mode → Cloud → enter Base URL `https://ark.cn-beijing.volces.com/api/v3`, the text/vision model IDs and API Key we provide above, tap Save, then go to the Studio (拆解) tab and import any short talking-head video. The review flow uses `cloudStandard`: only sampled frames/text are sent to that **user-configured third-party endpoint**, never the full video. Full-video `cloudDeep` requires a successful non-secret capability probe and separate upload/cost consent. The developer runs no server and receives nothing. Keys are stored in the Keychain. Privacy policy: `[URL]`.
