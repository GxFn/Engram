...
Engram is a "second brain" for short-video creators: import a video **from your own photo library**, and it runs speech-to-text and on-device visual understanding to break the video into its hook structure and storyboard, then lets you ask questions across everything you've saved. **It does not generate video** and provides no generative-AI service. No account/login is required.

**To test the main flow, use Cloud mode:** Settings → AI Mode → Cloud → enter Base URL `https://ark.cn-beijing.volces.com/api/v3`, the text/vision model IDs and API Key we provide above, tap Save, then go to the Studio (拆解) tab and import any short talking-head video. Only the frames/text used for analysis are sent to that **user-configured third-party endpoint**; the developer runs no server and receives nothing. Keys are stored in the Keychain. Privacy policy: `[URL]`.
